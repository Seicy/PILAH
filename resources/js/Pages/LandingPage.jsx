...
export default function LandingPage() {
                        ...
                        if (data.status === "scanned") {
                            ...
                            localStorage.setItem(
                                ...
                                JSON.stringify({
                                    ...
                                    kelas: data.captain.kelas,
                                    semester: data.captain.semester,
                                    uid: data.uid,
                                })
                            );
    ...
    return (
        <div className="min-h-screen bg-cover bg-center bg-no-repeat">
            ...
            {/* ================= SECTION 1 ================= */}
            <section id="home" className="relative w-full min-h-[120vh] pt-[96px] bg-white overflow-hidden">
                {/* BACKGROUND HANGGAR – SETENGAH LINGKARAN */}
                <div
                    className="absolute top-0 right-0 h-full w-[60%] overflow-hidden rounded-l-[50%]"
                >
                    <div
                ...
                <main
                    className="relative z-10 h-full px-16 grid lg:grid-cols-[35%_50%] gap-10 items-center w-full"
                >
                    {/* LEFT FRAME */}
                    <div className="flex flex-col gap-6">
                        ...
                        {/* RFID CARD */}
                        <div
                            className="bg-blue-900/90 backdrop-blur-lg p-14 rounded-3xl h-[600px]
                                       flex flex-col justify-between border border-blue-400/30"
                        >
            ...
            <section
                id="features"
                className="relative w-full min-h-[120vh] pt-[96px]
                           flex items-center justify-center -mt-32 -mb-20"
            >
                <div
                    className="py-24 px-6 max-w-7xl mx-auto bg-white shadow-2xl
                               rounded-[3rem] relative z-10 border border-gray-100"
                >
                    <h2
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
                        <FeatureCard
                            icon={<Zap size={40} />}
                            title="AI-Powered"
                            desc="Verifikasi alat otomatis menggunakan Computer Vision."
                        />
                        <FeatureCard
                            icon={<Radio size={40} />}
                            title="RFID Tech"
                            desc="Login cepat tanpa password, cukup tap kartu."
                        />
                        <FeatureCard
                            icon={<Wrench size={40} />}
                            title="Inventory"
                            desc="Manajemen Toolbox hanggar secara real-time."
                        />
                        <FeatureCard
                            icon={<Users size={40} />}
                            title="Tracking"
                            desc="Riwayat peminjaman per kelas & semester tercatat."
                        />
                    </div>
                </div>
            </section>
        ...
        >
            <div className="w-full flex justify-between items-center px-8">
                ...
                <div className="flex items-center gap-4">
                    <img
                        src={Kucing}
                        className="h-14 w-14 rounded-2xl shadow-lg border-2 border-white"
                    />
                    <div>
                        <h1
                            className={`font-black text-3xl tracking-tighter ${
                                solid ? "text-blue-900" : "text-blue-600"
                            }`}
                        >
                            PILAH
                        </h1>
                        <p
                            className={`text-[12px] font-bold uppercase tracking-[0.2em] ${
                                solid ? "text-blue-600" : "text-blue-900/90"
                            }`}
                        >
                            Hangar Maintenance
                        </p>
                ...
                {/* DESKTOP MENU */}
                <div
                    className={`hidden md:flex items-center space-x-8 font-black uppercase text-xl ${
            ...
            {/* MOBILE MENU */}
            {open && (
                <div className="md:hidden bg-white/95 backdrop-blur-md shadow-xl px-8 py-6 space-y-6 rounded-3xl mx-10 mt-4">
                    <button
                        onClick={() => scrollTo("home")}
                        className="block w-full text-left font-black uppercase text-xl text-slate-700"
                    >
                        Home
                    </button>
                    ...
                    <button
                        onClick={() => router.visit("/StoremanLogin")}
                        className="w-full px-6 py-3 rounded-xl bg-blue-600 text-white font-black uppercase text-xl"
                    >
                        Storeman Login
                    </button>
                </div>
    ...
    return (
        <div
            className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100
                       hover:border-blue-300 transition-all hover:shadow-xl group"
        >
            <div
                className="w-14 h-14 mb-6 text-blue-600 bg-blue-100 rounded-2xl
                           flex items-center justify-center
                           group-hover:bg-blue-600 group-hover:text-white transition-colors"
            >
                {icon}
            </div>
            <h4 className="font-black text-xl text-slate-900 mb-2">
                {title}
            </h4>
